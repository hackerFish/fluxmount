...
## 1. ClawHub（发布一次，让所有兼容 Agent Skills 的桌面 agent 都能安装）

> 注意：ClawHub 是 OpenClaw 生态的官方市场。发布后兼容 Agent Skills 的桌面 agent 能 `clawhub install` 装到，但**不会自动**同步到 WorkBuddy SkillHub / Qoder / 扣子这些独立市场——它们需你另外提交（见文末发布顺序）。腾讯 SkillHub 作为 ClawHub 国内镜像，很可能自动收录，使 WorkBuddy 用户也能搜到。
...
# ② 登录（用你的 GitHub 账号 OAuth；账号需注册满约 1 周/7 天以上，否则过不了上传门槛）
clawhub login
...
1. **GitHub**（已完成，已公开）
2. **ClawHub**（发布一次，Agent-Skills 兼容的桌面 agent 都能装；SkillHub 多同步收录，杠杆最高）
3. **Qoder**（补全跨平台收录）
